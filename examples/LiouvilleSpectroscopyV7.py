    ...
    def plot_pathways_grid(
        ...
        for row_idx, comp in enumerate(components):
            for col_idx, data in enumerate(columns_data):
                ...
                # Diagonale du cadran spectral, indépendante du zoom affiché.
                # Rephasing : haut-gauche -> bas-droite (y = -x).
                # Unrephasing : bas-gauche -> haut-droite (y = x).
                diagonal_extent = min(float(np.max(w)), abs(float(np.min(w))))
                ax.plot(
                    [0, diagonal_extent],
                    [0, y_zoom_sign * diagonal_extent],
                    color="white",
                    linestyle="--",
                    linewidth=1.0,
                    alpha=0.7,
