...
class System:
    ...
    def coherence2d(self, time_delays=None, diagram=None, scan_id=None, r=10, parallel=False):
        ...
        for i in range(scan_id[0]):
            ...
            if delta_t > 0:
                ...
                results = mesolve(self.H, rho, coherence_time, self.c_ops, _e_ops=self.e_ops)
        ...
        results = mesolve(self.H, rho, t_list, self.c_ops, _e_ops=self.e_ops)
        states = results.states
        ...
        for i in range(scan_id[0]+1, scan_id[1]):
            ...
            if delta_t > 0:
                ...
                states = [mesolve(self.H, state, coherence_time, self.c_ops, _e_ops=self.e_ops).states[-1] for state in states]
        ...
        states = [mesolve(self.H, state, t_list, self.c_ops, _e_ops=self.e_ops).states for state in states]
        ...
        while i < len(diagram):  
            ...
            if delta_t > 0:
                ...
                for s in range(len(states)):
                        for h in range(len(states[s])):
                            states[s][h] = mesolve(self.H, states[s][h], coherence_time, self.c_ops, _e_ops=self.e_ops).states[-1]
    ...
    def brcoherence2d(self, time_delays=None, diagram=None, scan_id=None, r=10, parallel=False):
        ...
        for i in range(scan_id[0]):
            ...
            if delta_t > 0:
                coherence_time = np.linspace(0, delta_t, int(delta_t * r))
                results = brmesolve(self.H, rho, coherence_time, self.c_ops, e_ops=self.e_ops)
                rho = results.states[-1]  # keeping only the last state
        ...
        t_list = np.linspace(0, delta_t, int(delta_t * r))
        results = brmesolve(self.H, rho, t_list, self.c_ops, e_ops=self.e_ops)
        states = results.states
        ...
        for i in range(scan_id[0] + 1, scan_id[1]):
            ...
            if delta_t > 0:
                ...
                states = [brmesolve(self.H, state, coherence_time, self.c_ops, e_ops=self.e_ops).states[-1] for state in states]
        ...
        states = [brmesolve(self.H, state, t_list, self.c_ops, e_ops=self.e_ops).states for state in states]
        ...
        while i < len(diagram):
            ...
            if delta_t > 0:
                ...
                for s in range(len(states)):
                    for h in range(len(states[s])):
                        states[s][h] = brmesolve(self.H, states[s][h], coherence_time, self.c_ops, e_ops=self.e_ops).states[-1]
    ...
    def para_mesolve(self, rho, only_last_state=True):
        ...
        """
        if only_last_state:
            return mesolve(self.H, rho, self.tlist, self.c_ops, _e_ops=self.e_ops).states[-1]
        else:
            return mesolve(self.H, rho, self.tlist, self.c_ops, _e_ops=self.e_ops).states
