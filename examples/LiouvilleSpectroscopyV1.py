...
from scipy.sparse.linalg import factorized, expm_multiply
...
class LiouvilleSpectroscopySolver:
    def __init__(self, params):
        ...
        self.profiling = {
            ...
            "resolvent_inversions": 0.0, # Time spent in np.linalg (G1, G2, G3)
            "feynman_pathways": 0.0,     # Time spent in multiplications (@)
            "k_integration": 0.0,        # Time spent in np.sum
            "total_scan": 0.0
        }
    ...
    def feed_model(self, H_model, interaction_op_array, c_ops_raw=None, interaction_type="dipole"):
        ...
        if H_model.ndim == 2:
            #np.newaxis (or None) add a dimension
            H_model = H_model[np.newaxis,:, :]
            interaction_op_array = interaction_op_array[np.newaxis, : , : ]

            if c_ops_raw is not None: 
                c_ops_raw = [c_op[np.newaxis, :, :] if c_op.ndim == 2 else c_op for c_op in c_ops_raw]
        

        print(f"--- Model Loading (Interaction Type : {interaction_type}) ---")
        self.N_k, self.dim, _ = H_model.shape

        # H_eigen is back to its 3D dimension (N_k, dim, dim)
        ...
        for i_k in range(self.N_k):
            ...
            # Reconstruction of H_eigen as a 3D diagonal matrix
            ...
            # 2. Projection of the interaction operator (now always 3D as input)
            O_k_raw = interaction_op_array[i_k]
            O_eigen = U_dag @ O_k_raw @ U
            
            # 3. Separation of J+ and J- depending on the interaction type
            if interaction_type == "dipole":
                # Strict RWA (Rotating Wave Approximation)
                for r in range(self.dim):
                    for c in range(self.dim):
                        # Numerical tolerance to avoid instabilities
                ...
                # Symmetry for the current (absorption if j > i)
                for i in range(self.dim):
            ...
            # 4. Projection of the dissipation (Lindblad) operators
            if c_ops_raw is not None:
    ...
    def _get_L_eff(self, w_probe):
        # We force L to be complex right from its creation!
        L = (self._spre(self.H_eigen) - self._spost(self.H_eigen)).astype(complex)
        for C_eigen, gamma in self.c_ops_eigen: 
    ...
    def generate_2D_spectra(self, w_list, tau2, k_array=None):
        ...
        if k_array is not None and self.N_k > 1:
            ...
            integration_factor = 1.0 # No integration for a system without k
        
        S3_reph = np.zeros((n_w, n_w), dtype=complex)
        S3_unreph = np.zeros((n_w, n_w), dtype=complex)

        print(f"Beginning 2D scan: {n_w}x{n_w} point grid...")
    ...
    def _print_profiling_report(self):
        ...
        categories = {
            "Global rephasing": self.profiling["rephasing_total"],
            "Global unrephasing": self.profiling["unrephasing_total"],
            "-> Inversions (G1, G2, G3)": self.profiling["resolvent_inversions"],
            "-> Feynman contractions": self.profiling["feynman_pathways"],
            "Integration (Sum k)": self.profiling["k_integration"]
        }
